...
class Requests:
    ...
    def do(
        ...
        try:
            ...
            if request_type == "GET":
                r = (
                    get(url=self.odrx_api + api_endpoint)
                    if isDroid is True
                    else get(url=self.osu_api + api_endpoint)
                )
                return r
            ...
            print(
                f"Error ocurred while trying to make a request to an API (isDroid = {isDroid}, err: {err})"
            )
            return None
