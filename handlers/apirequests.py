...
class DroidAPI:
    ...
    def get_profile(self, uid):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise Exception(f"Error while calling API. (Status Code = {response.status_code})")
    ...
    def login(self, username, passwd):
        ...
        try:
            ...
            print(f"Error occured while logging into droid servers: {err}, response: {response}")
            return False
        uid = int(data[0])
        return uid
        ...
        try:
            ...
            if beatmap_ids is not None:
                for beatmap_id in beatmap_ids:
                    ...
                    api_url = f'/wl_add?key={self.wl_key}&bid={beatmap_id}' if isAdd is True else api_url == f'/wl_rm?key={self.wl_key}&bid={beatmap_id}'
                    response = r().do(isDroid=True, request_type="GET", api_endpoint=api_url)
                    if response.status_code == 200:
    ...
    def wl_fromid(self, mapid, isAdd=True):
        try:
            api_url = f'/wl_add?key={self.wl_key}&bid={mapid}' if isAdd is True else api_url == f'/wl_rm?key={self.wl_key}&bid={mapid}'
            response = r().do(isDroid=True, request_type="GET", api_endpoint=api_url)
            if response.status_code == 200:
                print(f"{mapid} added to whitelist.") if isAdd is True else print(f"{mapid} removed from whitelist")
            else:
    ...
    def get_mapdata_fromhash(self, hash):
        try:
            ...
            if response.status_code == 200:
                ...
                raise Exception(f"Mapdata API is not 200 (status_code: {response.status_code})")
        except Exception as err:
